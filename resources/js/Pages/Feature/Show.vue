...
import { Comment, DeferredData, Feature, PaginatedData } from "@/types";
import { Deferred, Head, router, useForm, usePage, usePoll, WhenVisible } from "@inertiajs/vue3";
import { ref } from "vue";
...
const deleteCommentOnFrontEnd = function (comm: Comment): void {
  const index = props.comments.findIndex(x => x.id === comm.id)
  if (index !== -1) {
                ...
                <div class="flex gap-6">
                  <div>
                    <svg
                      ...
                      stroke-width="2"
                      stroke="currentColor"
                      class="h-[50px] w-[50px] rounded-full p-2 dark:bg-gray-600"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                  ...
                  >
                    <div>
                      <span class="font-bold mr-4">{{ comm.user.name }}</span>
                      <span class="text-xs dark:text-gray-400">{{ comm.created_at }}</span>
                    </div>
                    <div class="mt-2 italic">{{ comm.comment }}</div>


                  </div>
                </div>
                <button
                  v-if="user.id === comm.user.id"
                  :disabled="deleteForm.processing"
                  @click="deleteForm.delete(route('comments.destroy', comm.id), {
                    preserveScroll: true,
                    preserveState: true,
                    only: ['hh'],
                    onSuccess: () => deleteCommentOnFrontEnd(comm)
                  })"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
